...
public class MusicResourceReloadListener implements IResourceManagerReloadListener {
	...
	public void onResourceManagerReload(IResourceManager manager) {
		MusicChoicesMod.logger.debug("[Music Choices] Loading resources...");
	...
	private void loadMusicOptions(MusicPropertyList propertyList) {
		MusicChoicesMod.logger.debug("[Music Choices] Loading options.");
		
		if(propertyList.maxBackground > 0) {
			MusicChoicesMod.logger.debug("[Music Choices] Loaded max background as " + propertyList.maxBackground);
			MusicChoicesMod.maxBackground = propertyList.maxBackground;
		}
		
		if(propertyList.maxOvertop > 0) {
			MusicChoicesMod.logger.debug("[Music Choices] Loaded max overtop as " + propertyList.maxOvertop);
			MusicChoicesMod.maxOvertop = propertyList.maxOvertop;
		}
		
		if(propertyList.backgroundFade > 0.0001f) {
			MusicChoicesMod.logger.debug("[Music Choices] Loaded background fade as " + propertyList.backgroundFade);
			MusicChoicesMod.backgroundFade = propertyList.backgroundFade;
		}
		
		if(propertyList.fadeStrength > 0) {
			MusicChoicesMod.logger.debug("[Music Choices] Loaded fade strength as " + propertyList.fadeStrength);
			MusicChoicesMod.fadeStrength = propertyList.fadeStrength;
		}
		
		if(propertyList.menuTickDelayMin >= 0) {
			MusicChoicesMod.logger.debug("[Music Choices] Loaded min menu tick as " + propertyList.menuTickDelayMin);
		...
		if(propertyList.ingameTickDelayMin >= 0) {
			MusicChoicesMod.logger.debug("[Music Choices] Loaded min ingame tick as " + propertyList.ingameTickDelayMin);
			MusicChoicesMod.ingameTickDelayMin = propertyList.ingameTickDelayMin;
		}
		
		if(propertyList.ingameTickDelayMax >= 0) {
			MusicChoicesMod.logger.debug("[Music Choices] Loaded max ingame tick as " + propertyList.ingameTickDelayMax);
		...
		if(propertyList.doStopTracks) {
			MusicChoicesMod.logger.debug("[Music Choices] Loaded stop tracks as " + propertyList.stopTracks);
			MusicChoicesMod.stopTracks = propertyList.stopTracks;
		}
		
		if(propertyList.battleDistance >= 0) {
			MusicChoicesMod.logger.debug("[Music Choices] Loaded battle distance as " + propertyList.battleDistance);
			MusicChoicesMod.battleDistance = propertyList.battleDistance;
		}
		
		if(propertyList.doBattleMonsterOnly) {
			MusicChoicesMod.logger.debug("[Music Choices] Loaded battle monster only as " + propertyList.battleMonsterOnly);
